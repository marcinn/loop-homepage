...
`hugo.toml` sets `baseURL = "https://marcinn.github.io/loop-homepage/"` — every internal link/asset path needs the `/loop-homepage/` prefix in production. In templates, always pass **relative paths without a leading slash** through `relURL` (e.g. `"img/x.jpg" | relURL`, not `"/img/x.jpg" | relURL`) — a leading slash causes Hugo's `relURL` to skip prepending the baseURL's path component, which silently breaks every such link/asset under the `/loop-homepage/` subpath while still working locally at server root. This already caused one real bug (the footer's statute PDF link, and initially the `.project-card--highlight` image path).
